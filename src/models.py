...
querypacks = Table(
    'query_packs',
    Column('pack_id', db.Integer, ForeignKey('pack.id')),
    Column('query_id', db.Integer, ForeignKey('query.id'))
)

pack_tags = Table(
    'pack_tags',
    Column('tag_id', db.Integer, ForeignKey('tag.id')),
    Column('pack_id', db.Integer, ForeignKey('pack.id'), index=True)
)

node_tags = Table(
    'node_tags',
    Column('tag_id', db.Integer, ForeignKey('tag.id')),
    Column('node_id', db.Integer, ForeignKey('node.id'), index=True)
)

query_tags = Table(
    'query_tags',
    Column('tag_id', db.Integer, ForeignKey('tag.id')),
    Column('query_id', db.Integer, ForeignKey('query.id'), index=True)
)

file_path_tags = Table(
    'file_path_tags',
    Column('tag_id', db.Integer, ForeignKey('tag.id')),
    Column('file_path_id', db.Integer, ForeignKey('file_path.id'), index=True)
)
...
class Node(SurrogatePK, Model):
    ...
    def packs(self):
        return db.session.object_session(self) \
            .query(Pack) \
            .join(pack_tags, pack_tags.c.pack_id == Pack.id) \
            .join(node_tags, node_tags.c.tag_id == pack_tags.c.tag_id) \
            .filter(node_tags.c.node_id == self.id) \
            .options(db.lazyload('*'))
    ...
    def queries(self):
        return db.session.object_session(self) \
            .query(Query) \
            .join(query_tags, query_tags.c.query_id == Query.id) \
            .join(node_tags, node_tags.c.tag_id == query_tags.c.tag_id) \
            .filter(node_tags.c.node_id == self.id) \
            .options(db.lazyload('*'))
    ...
    def file_paths(self):
        return db.session.object_session(self) \
            .query(FilePath) \
            .join(file_path_tags, file_path_tags.c.file_path_id == FilePath.id) \
            .join(node_tags, node_tags.c.tag_id == file_path_tags.c.tag_id) \
            .filter(node_tags.c.node_id == self.id) \
            .options(db.lazyload('*'))
...
class DistributedQueryTask(SurrogatePK, Model):
    ...
    error_message = Column(db.String)
...
class User(UserMixin, SurrogatePK, Model):
    ...
    def set_password(self, password):
        ...
        if isinstance(hash_bytes, bytes):
            hash_bytes = hash_bytes.decode('utf-8')
        self.password = hash_bytes
        return
    ...
    def check_password(self, value):
        ...
        # Если в базе лежит строка вида "b'$2b$12...'", декодируем её в байты
        if isinstance(pw_hash, str) and pw_hash.startswith("b'"):
            import ast
            try:
                pw_hash = ast.literal_eval(pw_hash)
            except Exception:
                pw_hash = pw_hash.encode('utf-8')
        
        # Убеждаемся, что хеш - это байты
        if isinstance(pw_hash, str):
            pw_hash = pw_hash.encode('utf-8')
            
        # Убеждаемся, что вводимый пароль - это байты
        if isinstance(value, str):
            value = value.encode('utf-8')
            
        try:
            return bcrypt.check_password_hash(pw_hash, value)
        except ValueError:
            # Если всё равно ошибка (например, кривой хеш), возвращаем False
            return False
...
class CmdbValue(SurrogatePK, Model):
    ...
    object = relationship('CmdbObject', backref=db.backref('values', cascade='all, delete-orphan', lazy='dynamic'))
    
    attribute_id = reference_col('cmdb_attribute_dict', nullable=False)
    attribute = relationship('CmdbAttributeDict', backref=db.backref('values', cascade='all, delete-orphan', lazy='dynamic'))
